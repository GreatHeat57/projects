...
abstract class Skin_Base extends Elementor_Skin_Base {
	...
	public function register_controls( Widget_Base $widget ) {
		...
		$this->register_open_mode_control();
	...
	protected function register_open_mode_control() {
		$this->add_control(
			'open_mode',
			[
				'label' => __( 'Open Mode', 'elementor-pro' ),
				'type' => Controls_Manager::SELECT,
				'options' => [
					'permalink' => 'Permalink',
					'lightbox' => 'Lightbox'
	...
	protected function render_thumbnail() {
		...
		// by websoft4u
		if ($this->get_instance_value('open_mode') == 'lightbox') {
			$setting_key = $this->get_control_id( 'full' );
			$settings[ $setting_key ] = [
				'id' => get_post_thumbnail_id(),
			];
			$image = $settings[ $setting_key ];
			$image_url = Group_Control_Image_Size::get_attachment_image_src($image['id'], $setting_key, $settings);
			?>
			<a class="elementor-post__thumbnail__link" href="<?php echo $image_url; ?>" data-elementor-open-lightbox="yes" >
				<div class="elementor-post__thumbnail"><?php echo $thumbnail_html; ?></div>
			</a>
		<?php 
		}
		else { ?>
			<a class="elementor-post__thumbnail__link 987654" href="<?php echo $this->current_permalink; ?>">
				<div class="elementor-post__thumbnail"><?php echo $thumbnail_html; ?></div>
			</a>
		<?php 
		}
