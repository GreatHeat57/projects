...
function make_grid(data) {
    ...
    var cellsrenderer = function (row, columnfield, value, defaulthtml, columnproperties, rowdata) {
        if (columnfield == 'nameCode') {
            return '<a href="https://pampuni.com/' + APP_URL[1] + '/public/malfunctions/' + value.substring(4) + '/edit" style="float: right;padding: 9px 3px 0 0;">' + value + '</a>';
        }
            ...
            if (value != '' && ((value != 'draft' && (rowdata.user_role == 'admin' || rowdata.user_role == 'employee')) || value == 'publish' && (rowdata.user_role == 'client' || rowdata.user_role == 'contractor'))) {
                ...
                return "<img src='https://pampuni.com/" + APP_URL[1] + "/public/img/draft.png' width=40 style='margin:3px 5px 0 0; float: right;'>";
            }
                ...
                if (rowdata.last_comment_date != '' && rowdata.user == '' || strtotime(rowdata.last_visit_date) < strtotime(rowdata.last_comment_date)) {
                    return "<img src='https://pampuni.com/" + APP_URL[1] + "/public/img/exclamation.png\' width=30>";
                }
            }
        }
    }
    ...
    $('.jqx-icon-arrow-left').parent().click(function () {
        $('label.toggle i').click(function () {
            $(this).css('left', '9991px');
            $(this).parent().find('input').css('opacity', 0);
        });
        ...
        $('.toggle input').on('change', function() {
            ...
            var row_data = $('#form_grid').jqxGrid('getrowdata', $('#form_grid').jqxGrid('getselectedrowindex'));
            var type = row_data.nameCode.substr(0, 1) == 'A' ? 'malfunction' : 'guidance';
            var data = {
                id: $tr.data('id'),
                type: $tr.data('type'),
                status: $status,
    ...
    $('.jqx-icon-arrow-right').parent().click(function () {
        $('label.toggle i').click(function () {
            $(this).css('left', '9991px');
            $(this).parent().find('input').css('opacity', 0);
        });
