                        ...
                        <div class="product-attributes my-4">
                          @foreach ($attrArr as $attrKey => $attrVal)
                            <div class="form-group mb-2">
                              ...
                              <div class="">
                              @foreach ($attrVal as $optionKey => $optionVal)
                                <div class="custom-control custom-radio custom-control-inline">
                                  <input type="hidden" class="keys" value="">
                                  <input type="hidden" class="values" value="">
                                  <input type="radio" id="{{$attrKey}}{{ $optionKey }}" name="{{ $attrKey }}" class="custom-control-input mproduct-attr"  data-key="{{ $attrKey }}" value="{{ $optionVal }}" {{ $loop->first ? 'checked' : '' }}>
                                  <label class="custom-control-label" for="{{$attrKey}}{{ $optionKey }}">{{ $optionVal }}</label>
                                </div>
                              @endforeach
                              </div>
                            </div>
...
keys = $(".mproduct-attr:checked").map(function() {
   return $(this).data('key');
        ...
        $.ajax({
            ...
            data:{id:pid,qty:qty,size:sizes,color:colors,size_qty:size_qty,size_price:size_price,size_key:size_key,keys:keys,values:values},
